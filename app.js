...
/* ============================================================
   ...
   ============================================================ */
const DB = {
  KEYS:{courses:'cm_courses',tasks:'cm_tasks',notes:'cm_notes',grades:'cm_grades',schedules:'cm_schedules',settings:'cm_settings',seeded:'cm_seeded'},
...
/* ============================================================
   BERSIHKAN DATA (MULAI DARI KOSONG TOTAL)
   ============================================================ */
function seedIfEmpty(){
  // Sengaja disetel 'clean_v2' agar memori browser lama dipaksa bersih
  if(localStorage.getItem(DB.KEYS.seeded) === 'clean_v2') return;
  ...
  localStorage.setItem(DB.KEYS.seeded, 'clean_v2');
}
...
let state = { route:'home', taskFilterStatus:'Semua', schedDay: todayDayName(), courseDetail:null };
...
function pageCourses(){
  ...
  return `<div class="space-y-2">
    ${list.length ? list.map(c => `
      <div onclick="App.editCourse('${c.id}')" class="flex items-center gap-3 p-3 bg-base-100 border border-base-200 rounded-2xl shadow-sm cursor-pointer">
